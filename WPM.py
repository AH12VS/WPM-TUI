        ...
        for p_l in p_f:
            ...
            for p_f_l in p_l:
                if p_f_l not in user_passwd:
                    enc_to_normal = ced_inh_obj.bin_to_normal(
                        p_f_l)  # encode the data
...
if len(user_passwd) == 1:  # check if length of user_passwd not equal 0 (be any data in that)
    ...
    time.sleep(0.15)
    # get the input of user for open the software
    user_passwd_for_login = input("     >>> ")
    time.sleep(0.15)
    # loop work if user input not equal the real and true password
    while user_passwd_for_login != user_passwd[0]:
        free_space_after_user_passwd_for_login = 59-len(user_passwd_for_login)
        print(
            f"|    The {user_passwd_for_login} is incorrect{free_space_after_user_passwd_for_login*' '}|")
        time.sleep(0.15)
        print(f"|    Please enter your password{50*' '}|")
        time.sleep(0.15)
        user_passwd_for_login = input("     >>> ")
...
while True:
    ...
    if cond_for_work not in commands_list:
        ...
        print(
            f"|    There is no \"{cond_for_work}\" command{free_space_after_cond_for_work*' '}|")
        time.sleep(0.15)
    ...
    if cond_for_work in ["N", "n"]:
        ...
        time.sleep(0.15)
        # get name of password name of user
        input_passwd_name = input("     >>> ")
        time.sleep(0.15)
        ...
        while input_passwd_name in saved_passwd_name:
            ...
            time.sleep(0.15)
            print(f"|    These names are used before :{47*' '}|")
            time.sleep(0.15)
            print(f"|{80*'-'}|")
            time.sleep(0.15)
            ...
            print(f"|    Please Enter the Name of Password{43*' '}|")
            time.sleep(0.15)
            # get the user input for new name for the password
            input_passwd_name = input("     >>> ")
            time.sleep(0.15)
            print(f"|{80*' '}|")
            time.sleep(0.15)
        ...
        while ":" in input_passwd_name:
            print(
                f"|    Please enter the password name without \":\"{34*' '}|")
            time.sleep(0.15)
            # get user input again for name of password
            input_passwd_name = input("     >>> ")
            time.sleep(0.15)
            # check if user input name in saved_passwd_name
            while input_passwd_name in saved_passwd_name:
                ...
                print(f"|{80*' '}|")
                time.sleep(0.15)
                print(f"|    These names are used before : {46*' '}|")
                time.sleep(0.15)
                print(f"|{80*' '}|")
                time.sleep(0.15)
                print(f"|{80*'-'}|")
        ...
        while len(input_passwd_name) == 0:
            ...
            while ":" in input_passwd_name:
                print(
                    f"|    Please enter the password name without \":\"{32*' '}")
                time.sleep(0.15)
                # get user input again for name of password
                input_passwd_name = input("     >>> ")
                time.sleep(0.15)
        ...
        time.sleep(0.15)
        print(
            f"|    \"A\" for make password use numbers, charcters and symbols ((Strong){10*' '}|")
        time.sleep(0.15)
        print(f"|    \"N\" for make password only use numbers (Poor){31*' '}|")
        time.sleep(0.15)
        print(
            f"|    \"C\" for make password only use characters (Average)|(better than \"N\"){7*' '}|")
        time.sleep(0.15)
        print(
            f"|    \"S\" for make password only use symbols (Good)|(better than \"C\"){13*' '}|")
        time.sleep(0.15)
        print(f"|    \"M\" for make password manually{46*' '}|")
        time.sleep(0.15)

        # user input for choose a method
        ...
        while cond_for_select_mk_passwd not in ["A", "a", "N", "n", "C", "c", "S", "s", "M", "m"]:
            print(
                f"|    Please choose a mode in [A]/[N]/[C]/[S]/[M] modes{27*' '}|")
            time.sleep(0.15)
            cond_for_select_mk_passwd = input("     >>> ")
            time.sleep(0.15)
        ...
        if cond_for_select_mk_passwd in ["A", "a", "N", "n", "C", "c", "S", "s"]:
            ...
            time.sleep(0.15)
            print(
                f"|    Please enter a number grater than 3 and less than 61{24*' '}|")
            time.sleep(0.15)
            ...
            while True:
                ...
                time.sleep(0.15)
                try:
                    # try to convert the passwd_len to integer
                ...
                if type(passwd_len) == type(0):
                    ...
                    if passwd_len < 4:
                        print(
                            f"|    Please enter a number grater than 3{41*' '}|")
                        time.sleep(0.15)

                    if passwd_len >= 61:
                        print(
                            f"|    Please enter a number less than 61{42*' '}|")
                        time.sleep(0.15)
        ...
        if cond_for_select_mk_passwd in ["A", "a"]:
            passwd = n_mode_functions_obj.a_method(
                all, passwd_len, saved_passwd_value)

        if cond_for_select_mk_passwd in ["N", "n"]:
            passwd = n_mode_functions_obj.n_method(
                numbers, passwd_len, saved_passwd_value)

        if cond_for_select_mk_passwd in ["C", "c"]:
            passwd = n_mode_functions_obj.c_method(
                characters, passwd_len, saved_passwd_value)

        if cond_for_select_mk_passwd in ["S", "s"]:
            passwd = n_mode_functions_obj.s_method(
                symbols, passwd_len, saved_passwd_value)

        if cond_for_select_mk_passwd in ["M", "m"]:
            passwd = n_mode_functions_obj.m_method(all, saved_passwd_value)

        free_space_after_passwd_for_print = 75 - \
            (len(input_passwd_name)+len(passwd))
        # show the name of password and password
        print(
            f"|    {input_passwd_name}:{passwd}{free_space_after_passwd_for_print*' '}|")
        time.sleep(0.15)
        print(f"+{80*'-'}+")
        time.sleep(0.2)
        ...
        # save the file
        with open(f"{user_workspace_dir}/Documents/WPM/passwd_data", file_mode) as f:
            # code the password name and password value
            code_to_bin = ced_inh_obj.normal_to_bin(
                f"{input_passwd_name}:{passwd}")
            f.write(code_to_bin)
            f.write("\n")
    ...
    if cond_for_work in ["P", "p"]:
        print(f"+{80*'-'}+")
        time.sleep(0.2)
        cond_for_change_passwd = p_mode_functions_obj.p_method(
            user_passwd, ced_inh_obj, user_workspace_dir)
    ...
    if cond_for_work in ["R", "r"]:
        ...
        if len(saved_passwd_name) > 0:
            ...
            time.sleep(0.15)
            print(
                f"|    [RA]/[R] (\"RA\" for remove all passwords | \"R\" for remove{20*' '}|\n|    a specific password){56*' '}|")
            time.sleep(0.15)
            # get user input for choose a method
            cond_for_select_rm_mode = input("     >>> ")
            time.sleep(0.15)
        ...
        if cond_for_select_rm_mode in ["R", "r"]:
            r_mode_functions_obj.r_method(
                saved_passwd_name, saved_passwd_value, ced_inh_obj, user_workspace_dir)
        print(f"+{80*'-'}+")
        time.sleep(0.2)
